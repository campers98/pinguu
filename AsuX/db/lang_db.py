...
class Langs(MongoDB):
    ...
    def __ensure_in_db(self):
        ...
        if not chat_data:
            ...
            LOGGER.info(f"ɪɴɪᴛɪᴀʟɪᴢᴇᴅ ʟᴀɴɢᴜᴀɢᴇ ᴅᴏᴄᴜᴍᴇɴᴛ ғᴏʀ ᴄʜᴀᴛ {self.chat_id}")
            return new_data
        return chat_data
        ...
        for data in all_data:
            for key, val in keys.items():
                try:
                    ...
                    LOGGER.warning(
                        f"ʀᴇᴘᴀɪʀɪɴɢ ʟᴀɴɢs ᴅᴀᴛᴀʙᴀsᴇ - sᴇᴛᴛɪɴɢ '{key}:{val}' ғᴏʀ {data['_id']}",
                    )
...
def __pre_req_all_langs():
    start = time()
    LOGGER.info("sᴛᴀʀᴛɪɴɢ ʟᴀɴɢs ᴅᴀᴛᴀʙᴀsᴇ ʀᴇᴘᴀɪʀ...")
    collection = MongoDB(Langs.db_name)
    Langs.repair_db(collection)
    LOGGER.info(f"ᴅᴏɴᴇ ɪɴ {round((time() - start), 3)}s!")
