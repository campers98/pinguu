...
async def start_give_one(c: Abishnoi, m: Message):
    ...
    try:
        if m.chat.type != CT.PRIVATE:
            ...
            return
        g_id = await c.ask(text="sᴇɴᴅ ᴍᴇ ɴᴜᴍʙᴇʀ ᴏғ ɢɪᴠᴇᴀᴡᴀʏ", chat_id = m.chat.id, filters=filters.text)
        ...
        if curr:
            ...
            if curr["is_give"]:
                ...
                return
            while True:
                con = await c.ask(text="ʏᴏᴜ ɪɴғᴏ ɪs ᴀʟʀᴇᴀᴅʏ ᴘʀᴇsᴇɴᴛ ɪɴ ᴍʏ ᴅᴀᴛᴀʙᴀsᴇ ᴅᴏ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ\nʏᴇs : ᴛᴏ sᴛᴀʀᴛ ᴛʜᴇ ɢɪᴠᴇᴀᴡᴀʏ ᴡɪᴛʜ ᴘʀᴇᴠɪᴏᴜs ᴄᴏɴғɪɢᴜʀᴀᴛɪᴏɴs\nɴᴏ: ᴛᴏ ᴄʀᴇᴀᴛᴇ ᴏɴᴇ",chat_id = m.chat.id,filters=filters.text)
                ...
                if con.text.lower() == "yes":
                    ...
                    while True:
                        yes_no = await c.ask(text="ᴏᴋ.\nᴅᴏ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴀʟʟᴏᴡ ᴏʟᴅ ᴍᴇᴍʙᴇʀ ᴏғ ᴛʜᴇ ᴄʜᴀɴɴᴇʟ ᴄᴀɴ ᴠᴏᴛᴇ ɪɴ ᴛʜɪs ɢɪᴠᴇᴀᴡᴀʏ.\n**ʏᴇs: ᴛᴏ ᴀʟʟᴏᴡ**\n**ɴᴏ: ᴛᴏ ᴅᴏɴ'ᴛ ᴀʟʟᴏᴡ**\nɴᴏᴛᴇ ᴛʜᴀᴛ ᴏʟᴅ ᴍᴇᴀɴ ᴜsᴇʀ ᴡʜᴏ ɪs ᴘʀᴇsᴇɴᴛ ɪɴ ᴛʜᴇ ᴄʜᴀᴛ ғᴏʀ ᴍᴏʀᴇ ᴛʜᴀɴ 48 ʜᴏᴜʀs",chat_id = m.from_user.id,filters=filters.text)
        ...
        if uWu:
            while True:
                channel_id = await c.ask(text="ᴏᴋ....sᴇɴᴅ ᴍᴇ ɪᴅ ᴏғ ᴛʜᴇ ᴄʜᴀɴɴᴇʟ ᴀɴᴅ ᴍᴀᴋᴇ sᴜʀᴇ ɪ ᴀᴍ ᴀᴅᴍɪɴ ᴛʜᴇɪʀ. ɪғ ʏᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ɪᴅ ғᴏʀᴡᴀʀᴅ ᴀ ᴘᴏsᴛ ғʀᴏᴍ ʏᴏᴜʀ ᴄʜᴀᴛ.\nᴛʏᴘᴇ /cancel ᴄᴀɴᴄᴇʟ ᴛʜᴇ ᴄᴜʀʀᴇɴᴛ ᴘʀᴏᴄᴇss",chat_id = m.chat.id,filters=filters.text)
                if channel_id.text:
                    ...
                    try:
                        ...
                        try:
                            ...
                            if bot_stat in [CMS.ADMINISTRATOR,CMS.OWNER]:
                                ...
                                await c.send_message(m.chat.id,f"ʟᴏᴏᴋs ʟɪᴋᴇ ɪ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ᴀᴅᴍɪɴ ᴘʀɪᴠɪʟᴇɢᴇs ɪɴ ᴛʜᴇ ᴄʜᴀᴛ {c_id}\nᴍᴀᴋᴇ ᴍᴇ ᴀᴅᴍɪɴ ᴀɴᴅ ᴛʜᴇɴ sᴇɴᴅ ᴍᴇ ᴄʜᴀɴɴᴇʟ ɪᴅ ᴀɢᴀɪɴ")
                        except UserNotParticipant:
                            await c.send_message(m.chat.id,f"ʟᴏᴏᴋs ʟɪᴋᴇ ɪ ᴀᴍ ɴᴏᴛ ᴘᴀʀᴛ ᴏғ ᴛʜᴇ ᴄʜᴀᴛ {c_id}\n")
                        
                                
                    except ValueError:
                        await c.send_message(m.chat.id,"ᴄʜᴀɴɴᴇʟ ɪᴅ sʜᴏᴜʟᴅ ʙᴇ ɪɴᴛᴇɢᴇʀ ᴛʏᴘᴇ")
                    ...
                    if channel_id.forward_from_chat:
                        ...
                        await c.send_message(m.chat.id,f"ғᴏʀᴡᴀʀᴅ ᴍᴇ ᴄᴏɴᴛᴇɴᴛ ғʀᴏᴍ ᴄʜᴀᴛ ᴡʜᴇʀᴇ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ sᴛᴀʀᴛ ɢɪᴠᴇᴀᴡᴀʏ")
            f_c_id = c_id 
            await c.send_message(m.chat.id,"ᴄʜᴀɴɴᴇʟ ɪᴅ ʀᴇᴄᴇɪᴠᴇᴅ")
            while True:
                chat_id = await c.ask(text="sᴇɴᴅᴇ ᴍᴇ ɪᴅ ᴏғ ᴛʜᴇ ᴄʜᴀᴛ ᴀɴᴅ ᴍᴀᴋᴇ sᴜʀᴇ ɪ ᴀᴍ ᴀᴅᴍɪɴ ᴛʜᴇɪʀ. ɪғ ʏᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ɪᴅ ɢᴏ ɪɴ ᴛʜᴇ ᴄʜᴀᴛ ᴀɴᴅ ᴛʏᴘᴇ /id.\nᴛʏᴘᴇ /cancel ᴛᴏ ᴄᴀɴᴄᴇʟ ᴛʜᴇ ᴄᴜʀʀᴇɴᴛ ᴘʀᴏᴄᴇss",chat_id = m.chat.id,filters=filters.text)
                if chat_id.text:
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                await c.send_message(m.chat.id,f"ʟᴏᴏᴋs ʟɪᴋᴇ ᴄʜᴀᴛ ᴅᴏᴇsɴ'ᴛ ᴇxɪsᴛ: {e}")
                    except ValueError:
                        await c.send_message(m.chat.id,"ᴄʜᴀᴛ ɪᴅ sʜᴏᴜʟᴅ ʙᴇ ɪɴᴛᴇɢᴇʀ ᴛʏᴘᴇ")
                    try:
                        bot_stat = (await c.get_chat_member(s_c_id, Abishnoi.id)).status
                        if bot_stat in [CMS.ADMINISTRATOR,CMS.OWNER]:
                            break
                        else:
                            await c.send_message(m.chat.id,f"ʟᴏᴏᴋs ʟɪᴋᴇ ɪ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ᴀᴅᴍɪɴ ᴘʀɪᴠɪʟᴇɢᴇs ɪɴ ᴛʜᴇ ᴄʜᴀᴛ {s_c_id}\nᴍᴀᴋᴇ ᴍᴇ ᴀᴅᴍɪɴ ᴀɴᴅ ᴛʜᴇɴ sᴇɴᴅ ᴍᴇ ᴄʜᴀɴɴᴇʟ ɪᴅ ᴀɢᴀɪɴ")
                    except UserNotParticipant:
                        await c.send_message(m.chat.id,f"ʟᴏᴏᴋs ʟɪᴋᴇ ɪ ᴀᴍ ɴᴏᴛ ᴘᴀʀᴛ ᴏғ ᴛʜᴇ ᴄʜᴀᴛ {s_c_id}\n")
                
            await c.send_message(m.chat.id,"ᴄʜᴀᴛ ɪᴅ ʀᴇᴄᴇɪᴠᴇᴅ")
                
            link = await c.export_chat_invite_link(cc_id)

            yes_no = await c.ask(text="ᴅᴏ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴀʟʟᴏᴡ ᴏʟᴅ ᴍᴇᴍʙᴇʀ ᴏғ ᴛʜᴇ ᴄʜᴀɴɴᴇʟ ᴄᴀɴ ᴠᴏᴛᴇ ɪɴ ᴛʜɪs ɢɪᴠᴇᴀᴡᴀʏ.\n**ʏᴇs: ᴛᴏ ᴀʟʟᴏᴡ**\n**ɴᴏ: ᴛᴏ ᴅᴏɴ'ᴛ ᴀʟʟᴏᴡ**\nɴᴏᴛᴡ ᴛʜᴀᴛ ᴏʟᴅ ᴍᴇᴀɴ user ᴡʜᴏ ɪs ᴘʀᴇsᴇɴᴛ ɪɴ ᴛʜᴇ ᴄʜᴀᴛ ғᴏʀ ᴍᴏʀᴇ ᴛʜᴀɴ 48 ʜᴏᴜʀs",chat_id = m.from_user.id,filters=filters.text)
            if yes_no.text.lower() == "yes":
                is_old = 0
            elif yes_no.text.lower() == "no":
                is_old = 1
...
"""
    ...
    kb = IKM([[IKB("ᴊᴏɪɴ ᴛʜᴇ ᴄʜᴀᴛ", url=link)],[IKB("sᴛᴀʀᴛ ᴛʜᴇ ʙᴏᴛ", url=f"https://{Abishnoi.username}.t.me/")]])
    try:
    ...
    name = c_in.title
    await m.reply_text(f"✨ ɢɪᴠᴇᴀᴡᴀʏ ᴘᴏsᴛ ʜᴀs ʙᴇᴇɴ sᴇɴᴛ ᴛᴏ [{name}]({c_in.invite_link})", disable_web_page_preview=True, reply_markup=IKM([[IKB("ɢᴏ ᴛᴏ ᴘᴏsᴛ", url=pin.link)]]))
...
"""
    ...
    try:
        ...
        await m.reply_text(f"ғᴀɪʟᴇᴅ ᴛᴏ ɢᴇᴛ ᴍᴇssᴀɢᴇ ғᴏʀᴍ ᴛʜᴇ ᴄʜᴀᴛ ɪᴅ {c_id}. ᴅᴜᴇ ᴛᴏ ғᴏʟʟᴏᴡɪɴɢ ᴇʀʀᴏʀ\n{e}")
        return False
    try:
        if mess.caption:    
        ...
        await m.reply_text(f"ʜᴇʀᴇ ɪs ᴛʜᴇ ᴛᴇxᴛ ʏᴏᴜ ᴄᴀɴ ᴇᴅɪᴛ ᴛʜᴇ ᴍᴇssᴀɢᴇ ʙʏ ʏᴏᴜʀ sᴇʟғ\n`{txt}`\nsᴏʀʀʏ ғᴏʀ ɪɴᴄᴏɴᴠᴇɴɪᴇɴᴄᴇ")
        return False
...
"""
        ...
        to_key = ["ᴊᴀɪ ʜɪɴᴅ", "ᴊᴀɪ ᴊᴀᴡᴀᴀɴ","ᴊᴀɪ ʙʜᴀʀᴀᴛ", "ᴊᴀɪ sʜʀᴇᴇ ʀᴀᴍ", "ᴊᴀɪ sʜʀᴇᴇ sʜʏᴀᴍ", "ᴊᴀɪ sʜʀᴇᴇ ᴋʀɪsʜɴ", "ᴊᴀɪ sʜʀᴇᴇ ʀᴀᴅʜᴇ", "ʀᴀᴅʜᴇ ʀᴀᴅʜᴇ", "sᴀᴍʙʜᴜ", "ᴊᴀɪ ᴍᴀᴛᴀ ᴅɪ", "ᴊᴀɪ ᴍᴀʜᴀᴋᴀᴀʟ", "ᴊᴀɪ ʙᴀᴊᴀʀᴀɴɢʙᴀʟɪ"]
        key = choice(to_key)
        high = max_value
...
async def register_user(c: Abishnoi, m: Message):
    ...
    if not curr:
        await m.reply_text("ɴᴏ ɢɪᴠᴇᴀᴡᴀʏ ᴛᴏ ᴘᴀʀᴛɪᴄɪᴘᴀᴛᴇ ɪɴ.\nᴏʀ ᴍᴀʏ ʙᴇ ᴇɴᴛʀɪᴇs ᴀʀᴇ ᴄʟᴏsᴇᴅ ɴᴏᴡ")
        return
    ...
    if not curr["is_give"]:
        ...
        await m.reply_text("ʏᴏᴜ ᴀʀᴇ ʟᴀᴛᴇ,\nᴇɴᴛʀɪᴇs ᴀʀᴇ ᴄʟᴏsᴇᴅ 🫤\nᴛʀʏ ᴀɢᴀɪɴ ɪɴ ɴᴇxᴛ ɢɪᴠᴇᴀᴡᴀʏ")
        return
    c_id = curr["chat_id"]
    ...
    try:
        ...
        await m.reply_text("sᴛᴀʀᴛ ᴛʜᴇ ʙᴏᴛ ғɪʀsᴛ\nᴀɴᴅ ᴛʀʏ ᴀɢᴀɪɴ",reply_markup=IKM([[IKB("sᴛᴀʀ ᴛʜᴇ ʙᴏᴛ", url=f"https://{Abishnoi.username}.t.me/")]]))
        return
    curr = GA.give_info(m.chat.id)
    c_id = curr["chat_id"]
    ...
    if len(rejoin_try):
        try:
            if q.from_user.id in rejoin_try[c_id]:
                await q.answer("ʏᴏᴜ ᴄᴀɴ'ᴛ ᴠᴏᴛᴇ. ʙᴇᴄᴀᴜsᴇ ʏᴏᴜ'ʀᴇ ʀᴇᴊᴏɪɴᴇᴅ ᴛʜᴇ ᴄʜᴀᴛ ᴅᴜʀɪɴɢ ɢɪᴠᴇᴀᴡᴀʏ")
                return
        except KeyError:
            pass
    is_old = curr["is_new"]
...
async def rejoin_try_not(c:Abishnoi, m: Message):
    ...
    if len(voted_user):
        if Captain in voted_user[m.chat.id]:
            ...
            await c.send_message(GB,f"ᴏɴᴇ ᴜsᴇʀ ᴡʜᴏ ʜᴀᴠᴇ ᴠᴏᴛᴇᴅ ʏᴏᴜ ʟᴇғᴛ ᴛʜᴇ ᴄʜᴀᴛ sᴏ ʜɪs ᴠᴏᴛᴇ ɪs ʀᴇᴅᴜᴄᴇᴅ ғʀᴏᴍ ʏᴏᴜʀ ᴛᴏᴛᴀʟ ᴠᴏᴛᴇs.\nɴᴏᴛᴇ ᴛʜᴀᴛ ʜᴇ ᴡɪʟʟ ɴᴏᴛ ᴀʙʟᴇ ᴛᴏ ᴠᴏᴛᴇ ɪғ ʜᴇ ʀᴇᴊᴏɪɴs ᴛʜᴇ ᴄʜᴀᴛ\nʟᴇғᴛ ᴜsᴇʀ : {Captain}")
            try:
