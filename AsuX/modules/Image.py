...
LOW_VRAM_MODE = "true"  # (os.getenv('LOW_VRAM', 'true').lower() == 'true')
USE_AUTH_TOKEN = "true"  # (os.getenv('USE_AUTH_TOKEN', 'true').lower() == 'true')
SAFETY_CHECKER = "true"  # (os.getenv('SAFETY_CHECKER', 'true').lower() == 'true')
HEIGHT = "512"
WIDTH = "512"
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "TRYAGAIN":
        if (
            replied_message.photo is not None
            and len(replied_message.photo) > 0
            and replied_message.caption is not None
        ):
